...
| **LaunchAgents** | Every plist in `~/Library/LaunchAgents` with its program, whether launchd has it loaded, and its PID. Load and unload go through `launchctl bootstrap`/`bootout` for your own gui domain only; Dockmaster's own agent is off limits. |
| **Agent Watch** | Coding agents running right now (Claude Code, Codex, Gemini CLI, OpenCode, Cursor Agent, Kiro CLI, Aider, Amp, Copilot CLI, Goose) with the repo each one is in, plus the last seven days of sessions (prompts, tools, tokens, cost) read from Claude Code, Codex, and OpenCode history. On this Mac each folder's newest session also lists the servers running there, labeled as the agent's (a descendant of a running agent), left behind (no terminal, started during an agent session, no agent there now, sessions quiet for ten minutes), or unclaimed, and linked worktrees show uncommitted paths. A Left behind banner counts those servers plus merged, clean worktrees untouched for a day, and opens `/cleanup`, which shows why each one was flagged and the exact command Clean up will run; Clean up re-scans, refuses while an agent is in the folder, sends SIGTERM, and runs `git worktree remove` without `--force`. Works on remote Macs through your own SSH, without the leftover join; nothing leaves your machines. |
| **Receipt** | The last seven days as a shareable receipt: sessions per agent, prompts, tool calls, tokens, lines changed, top projects, recorded cost, and what Dockmaster stopped, removed, and reclaimed. Copy it as an image or download a PNG; project names are hidden unless you switch them on. Cleanup counts come from `~/.dockmaster/actions.json`, written when a stop, worktree removal, or disk clean succeeds and kept for 90 days. Local-only; needs Agent Watch on. |
