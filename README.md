...
| **Agent Watch** | Coding agents running right now (Claude Code, Codex, Gemini CLI, OpenCode, Cursor Agent, Kiro CLI, Aider, Amp, Copilot CLI, Goose) with the repo each one is in, plus today's token use per project read from Claude Code and Codex session logs. Fully local; nothing is sent anywhere. |
